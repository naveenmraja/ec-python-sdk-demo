class JuspayException(Exception):
    ...
    def __str__(self):
        if self._message is not None:
            return u"Request failed. Message : {1}".format(self.http_status, self._message)
        elif self.json_body is not None:
            return u"Request failed with httpResponseCode : {0}\n{1}".format(self.http_status, self.json_body)
        else:
            return u"Request failed with unknown reason, please contact support@juspay.in."
